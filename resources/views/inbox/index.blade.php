                            ...
                            <div class="media {{ $messageGroup->first()->shouldAddUnreadClass() ? 'unread' : '' }}">
                                <div class="media-left">
                                    <a href="">
                                        @if(Auth::id() == $messageGroup->last()->from_user_id)
                                            <img width="36" src="{{ $messageGroup->last()->toUser->avatar }}">
                                        @else
                                            <img width="36" src="{{ $messageGroup->last()->fromUser->avatar }}">
                                        @endif
                                    </a>
                                </div>
                                <div class="media-body">
                                    <div class="media-heading">
                                        ...
                                        <p><a href="/inbox/{{ $messageGroup->first()->dialog_id }}">{{ $messageGroup->first()->body }}</a></p>
                                    </div>
                                </div>
                            </div>
